...
import { ShoppingCart, ShieldCheck, Menu, X, UserRound, LogOut, PackageOpen, Heart, Bell } from "lucide-react";
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <>
              ...
              >{user.user_metadata?.avatar_url ? <img src={user.user_metadata.avatar_url} alt="الصورة الشخصية" className="h-5 w-5 rounded-full object-cover" /> : <UserRound className="h-4 w-4" />}<span className="hidden sm:inline">{t("profile")}</span></Link>
              <Link
                to="/purchases"
                className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-2 text-sm font-medium shadow-sm transition hover:-translate-y-0.5 hover:border-primary/30 hover:bg-primary/5"
              >
